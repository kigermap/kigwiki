...
│   │   ├── citation/index.md    # 引用规范
│   │   ├── faq/index.md         # 常见问题
│   │   └── license/index.md     # 开源协议说明
│   ├── zh-Hant/                 # 繁体中文
│   ├── en/                      # English
│   ├── ja/                      # 日本語
│   ├── ru/                      # Русский
...
线下体验资料位于 `docs/zh-Hans/experiences/`，提供目录、独立场所详情和检索复核记录。按官方服务项目记录原币报价、预约条件和来源日期；官网可访问不等于实时营业确认。详情为人工维护的 Markdown，其他语言沿用简体中文回退。混合经营主体只收录人脸／动漫角色型 kigurumi 体验，不混入兽装或普通吉祥物项目。

体验目录采用图片卡片，四家详情包含官方照片、交通与公开地址、业务介绍及可核实的历史／活动记录。照片保存在 `docs/assets/images/experiences/`，`manifest.json` 记录原图来源、权利说明、日期、尺寸和校验值；不随本站文字许可再授权。图文样式位于 `docs/assets/stylesheets/experiences.css`。新增图像需人工确认与场所相关，不能将网站模板图库或历史照片描述为当前店内实景。
...
- `docs/zh-Hans/citation/index.md`：引用规范，说明页面、事件、地点、人物、来源和素材如何引用。
- `docs/zh-Hans/faq/index.md`：常见问题，说明项目介绍、成立目的、加入方式、免费开源和隐私原则。
- `docs/zh-Hans/license/index.md`：开源协议说明，区分代码 MIT License、原创文字 CC BY 4.0、第三方素材和投稿资料授权边界。
...
- `docs/robots.txt` 和 `docs/llms.txt` 分别服务搜索引擎抓取与 AI/生成式搜索摘要。
...
站点通过 `mkdocs.yml` 和 `scripts/site_hooks.py` 维护基础 SEO/GEO 信号：

- `site_url` 用于生成正确的 sitemap URL。
- 每个 HTML 页面会补充 canonical、Open Graph、Twitter Card 和 JSON-LD。
- `robots.txt` 指向生产 sitemap。
- `llms.txt` 为生成式搜索和 AI 摘要提供项目说明、核心链接、引用边界和参与入口。
- 多语言页面使用 `hreflang` 指向对应语言版本。

当前生产域名为 `https://kigwiki.com/`。如果后续再次切换域名，需要同步更新 `mkdocs.yml`、`docs/robots.txt`、`docs/llms.txt`、`scripts/site_hooks.py` 和引用规范示例中的站点 URL。

## 授权策略

项目采用分层授权，避免把代码开源和资料公开混为一谈：

- 网站代码、样式、脚本、构建配置和工程工具：MIT License，见 `LICENSE`。
- Kigurumi 编年志原创说明文字、页面摘要、整理性说明：默认 CC BY 4.0，转载或改写时需署名、附链接、保留语境并标注改动。
- 原创表格、时间线和整理性元数据：默认 CC BY 4.0，但复用前仍应回到原始来源核对事实。
- 照片、访谈、海报、截图、二维码、社交平台内容、用户投稿和第三方资料：不随 MIT 或默认 CC BY 4.0 自动开放，按页面标注、投稿约定或原权利人授权使用。
- 未核实、待授权、需匿名、需打码或仅作内部线索的资料：不应公开转载，也不应作为确定事实引用。

授权边界的仓库说明见 `LICENSE-CONTENT.md`，站内说明见 `docs/zh-Hans/license/index.md`。
